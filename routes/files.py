...
from app import validate_path, json_ok, json_err, get_req_path, PROJECT_DIR, MAX_FILE_SIZE
import os, datetime, subprocess, shlex, re
...
def api_files_read():
    path = validate_path(get_req_path())
    if not os.path.isfile(path): return json_err("Invalid file path", 400, content="")
    if os.path.getsize(path) > MAX_FILE_SIZE: return json_err("File too large (>1MB)", 413, content="")
    try:
...
def api_files_write():
    ...
    if 'path' not in data: return json_err("No path provided", 400)
...
def api_files_create():
    ...
    if not filename: return json_err("No filename provided", 400)
    dir_path = validate_path(data.get('path', ''))
    new_path = os.path.join(dir_path, os.path.basename(filename))
    if not new_path.startswith(PROJECT_DIR): return json_err("Cannot create outside project directory", 403)
    if os.path.exists(new_path): return json_err("File already exists", 409)
    try:
...
def api_files_download():
    path = validate_path(get_req_path())
    if not os.path.isfile(path): return json_err("File not found", 404)
    return send_from_directory(
        os.path.dirname(path), os.path.basename(path),
        as_attachment=True, download_name=os.path.basename(path)
    )

@files_bp.route('/api/files/upload', methods=['POST'])
def api_files_upload():
    target = validate_path(request.form.get('path', ''))
    if 'file' not in request.files: return json_err("No file part", 400)
    file = request.files['file']
    if not file.filename: return json_err("No selected file", 400)
    save_path = os.path.join(target, os.path.basename(file.filename))
    if not save_path.startswith(PROJECT_DIR): return json_err("Invalid upload path", 403)
    try:
        file.save(save_path)
...
def project_info():
    ...
    try:
        ...
        if r.returncode == 0: tree_str = r.stdout
        else: tree_str = "Command 'tree' not found"
    except Exception:
